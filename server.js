...
app.post('/run-r-script', (req, res) => {
    const scriptPath = path.join(__dirname, 'MetadataProcess', 'MetadataExtraction.R');
    ...
    exec(command, (error, stdout, stderr) => {
        ...
        console.log(`Salida del script de R:\n${stdout}`);
    });
    res.status(200).send('Script de R ejecutado');
});
...
app.post('/run-python-script', (req, res) => {
    ...
    exec(command, (error, stdout, stderr) => {
        ...
        const urlPattern = /https?:\/\/[^\s]+/; // Expresión regular para buscar un URL
        const match = stdout.match(urlPattern);

        if (match) {
            const url = match[0]; // El primer URL que se encuentra 

            // Ahora, guardamos la URL en la base de datos
            console.log(url);
            res.status(200).send(url); // Enviamos el URL al cliente

        } else {
            console.error('No se encontró un URL en la salida del script');
            res.status(500).send('No se encontró un URL en la salida');
        } 
    });
});
...
app.post('/save-csv-file', (req, res) => {
    ...
    // Validar que los datos y el nombre de archivo están presentes
    if (!data || !fileName) {
      return res.status(400).json({ error: 'Se requieren los datos y el nombre del archivo' });
    }
    ...
    try {
      ...
      return res.status(500).json({ error: 'Error al convertir los datos a CSV' });
    }
